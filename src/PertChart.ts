...
import { ActivityWithTimes, NodePERT, LinkPERT } from './interfaces';
import { numToLetter } from './util';

const circleRadius = 40;
const redColor = '#ff441f';
...
export const generatePERTGraph = (activities: ActivityWithTimes[]) => {
  ...
  node // Earliest start time
    ...
    .text((d) => 'T_i: ' + d.earliestStartTime.toFixed(2));
  ...
  node // Latest start time
    ...
    .attr('text-anchor', 'middle')
    .style('font-size', '10px')
    .text((d) => 'T_f: ' + d.latestStartTime.toFixed(2));
